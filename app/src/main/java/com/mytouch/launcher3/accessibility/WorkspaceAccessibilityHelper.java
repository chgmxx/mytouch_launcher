...
public class WorkspaceAccessibilityHelper extends DragAndDropAccessibilityDelegate {
    ...
    protected String getConfirmationForIconDrop(int id) {
        ...
        View child = mView.getChildAt(x, y);
        if (child == null || child == dragInfo.item) {
            return mContext.getString(com.mytouch.launcher3.R.string.item_moved);
        } else {
            ItemInfo info = (ItemInfo) child.getTag();
            if (info instanceof AppInfo || info instanceof ShortcutInfo) {
                return mContext.getString(com.mytouch.launcher3.R.string.folder_created);

            } else if (info instanceof FolderInfo) {
                return mContext.getString(com.mytouch.launcher3.R.string.added_to_folder);
            }
        }
        return "";
    }
    ...
    protected String getLocationDescriptionForIconDrop(int id) {
        ...
        if (child == null || child == dragInfo.item) {
            if (mView.isHotseat()) {
                return mContext.getString(com.mytouch.launcher3.R.string.move_to_hotseat_position, id + 1);
            } else {
                return mContext.getString(com.mytouch.launcher3.R.string.move_to_empty_cell, y + 1, x + 1);
            }
        } else {
            ItemInfo info = (ItemInfo) child.getTag();
            if (info instanceof ShortcutInfo) {
                return mContext.getString(com.mytouch.launcher3.R.string.create_folder_with, info.title);
            } else if (info instanceof FolderInfo) {
                if (TextUtils.isEmpty(info.title)) {
                    ...
                    if (firstItem != null) {
                        return mContext.getString(com.mytouch.launcher3.R.string.add_to_folder_with_app, firstItem.title);
                    }
                }
                return mContext.getString(com.mytouch.launcher3.R.string.add_to_folder, info.title);
            }
        }
        return "";
    }
